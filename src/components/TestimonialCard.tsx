...
const TestimonialCard = ({ name, role, company, testimonial, avatar }: TestimonialCardProps) => {
  return (
    <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-pink-300/20">
      <div className="flex items-center mb-4">
        {[...Array(5)].map((_, i) => (
          <Star key={i} className="h-4 w-4 text-yellow-400 fill-current" />
        ))}
      </div>
      <p className="text-pink-100 mb-6 italic">"{testimonial}"</p>
      <div className="flex items-center">
        ...
        <div>
          ...
          <p className="text-pink-300 text-sm">{role} at {company}</p>
        </div>
      </div>
    </div>
  );
